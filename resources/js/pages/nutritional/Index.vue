    ...
    <AppLayout>
        <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                    ...
                    <div class="relative">
                        ...
                        <div class="relative">
                            ...
                            <input
                                ...
                                class="w-full rounded-md border border-gray-300 py-2 pr-3 pl-10 transition-colors focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none sm:text-sm"
                            />
                        </div>
                    </div>
                </div>
            ...
            <div class="md:col-span-3">
                <div class="overflow-hidden rounded-lg border border-gray-200 shadow-sm">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="w-1/3 px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Ingrediente</th>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <template v-for="(ingredient, iIndex) in groupedIngredients.data" :key="ingredient.id">
                                <tr
                                    v-for="(factor, fIndex) in ingredient.nutritional_factors"
                                    :key="factor.id"
                                    :class="[
                                        fIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50/30',
                                        'border-b border-gray-100 transition-colors hover:bg-blue-50/40',
                                    ]"
                                >
                                    <td
                                        v-if="fIndex === 0"
                                        :rowspan="ingredient.nutritional_factors.length"
                                        class="border-r border-gray-200 bg-white px-6 py-4 align-middle text-sm font-semibold text-gray-800 shadow-[inset_-2px_0_4px_rgba(0,0,0,0.01)]"
                                    >
                                    ...
                                    <td class="px-6 py-3 text-right text-sm font-medium whitespace-nowrap">
                                        <div class="flex justify-end gap-2">
                                            <button
                                                ...
                                                class="rounded p-1.5 text-blue-600 transition-colors hover:bg-blue-100 hover:text-blue-800"
                                                title="Editar"
                                            >
                            ...
                            <tr v-if="groupedIngredients.data.length === 0">
                                <td colspan="4" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-500">
                                        ...
                                        <p class="mt-1 text-sm">Intente buscar con otro término o agregue nuevos factores.</p>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                ...
                <!-- Paginación -->
                <div
                    v-if="groupedIngredients.data.length > 0"
                    class="mt-4 flex items-center justify-between rounded-lg border border-gray-200 bg-white px-4 py-3 shadow-sm sm:px-6"
                >
                    <div class="flex flex-1 justify-between sm:hidden">
                        <Link
                            v-if="groupedIngredients.prev_page_url"
                            :href="groupedIngredients.prev_page_url"
                        ...
                        <Link
                            v-if="groupedIngredients.next_page_url"
                            :href="groupedIngredients.next_page_url"
                    ...
                    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Mostrando <span class="font-medium">{{ groupedIngredients.from }}</span> a
                                <span class="font-medium">{{ groupedIngredients.to }}</span> de
        ...
        <div v-if="isModalOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    class="inline-block transform overflow-visible rounded-2xl border border-gray-100 bg-white text-left align-bottom shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle"
                >
                    <form @submit.prevent="submitForm">
                        <div class="px-8 pt-8 pb-6">
                            <div class="flex items-center gap-3 border-b border-gray-100 pb-5">
                                ...
                                <h3 class="text-xl font-bold tracking-tight text-gray-800" id="modal-title">
                            ...
                            <div class="mt-6 grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                <div class="relative sm:col-span-2">
                                    <label class="mb-1.5 block text-sm font-semibold text-gray-700">Ingrediente</label>
                                    <div class="relative">
                                        <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <input
                                            ...
                                            class="block w-full rounded-xl border border-gray-200 bg-gray-50/50 py-3 pr-4 pl-10 text-sm text-gray-900 transition-all duration-200 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
                                            required
                                        />
                                    </div>
                                    ...
                                    <div
                                        ...
                                        class="absolute z-50 mt-2 max-h-60 w-full origin-top transform overflow-x-hidden overflow-y-auto rounded-xl border border-gray-100 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-all"
                                    >
                                        <div
                                            v-for="ing in filteredIngredients"
                                            :key="ing.id"
                                            @click="selectIngredient(ing)"
                                            class="cursor-pointer border-b border-gray-50 px-4 py-3 text-sm font-medium text-gray-600 transition-colors last:border-0 hover:bg-blue-50/80 hover:text-blue-700"
                                        >
                                            {{ ing.name }}
                                        </div>
                                        <div
                                            v-if="filteredIngredients.length === 0"
                                            class="flex flex-col items-center gap-2 px-4 py-4 text-center text-sm text-gray-500"
                                        >
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <p v-if="form.errors.ingredient_id" class="mt-2 flex items-center gap-1 text-sm font-medium text-red-500">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                            <path
                                                fill-rule="evenodd"
                                ...
                                <div>
                                    <label class="mb-1.5 block text-sm font-semibold text-gray-700">NFactorCal</label>
                                    <div class="relative">
                                        <input
                                            type="number"
                                            step="0.01"
                                            v-model="form.nfactorcal"
                                            class="block w-full rounded-xl border border-gray-200 bg-gray-50/50 px-4 py-3 text-sm text-gray-900 transition-all duration-200 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
                                            placeholder="0.00"
                                        />
                                    </div>
                                    <p v-if="form.errors.nfactorcal" class="mt-2 text-sm font-medium text-red-500">{{ form.errors.nfactorcal }}</p>
                                </div>

                                <div>
                                    <label class="mb-1.5 block text-sm font-semibold text-gray-700">Composición</label>
                                    <div class="relative">
                                        <input
                                            type="number"
                                            step="0.01"
                                    ...
                                    <p v-if="form.errors.composition" class="mt-2 text-sm font-medium text-red-500">{{ form.errors.composition }}</p>
                                </div>
                            </div>
                        </div>

                        <div class="rounded-b-2xl border-t border-gray-100 bg-gray-50/80 px-8 py-5 sm:flex sm:flex-row-reverse">
                            <button
                                type="submit"
                                :disabled="form.processing"
                                class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-transparent bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition-all hover:bg-blue-700 focus:ring-4 focus:ring-blue-500/20 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 sm:ml-3 sm:w-auto"
                            >
                                <svg
                            ...
                            <button
                                type="button"
                                @click="closeModal"
                                class="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white px-6 py-3 text-sm font-semibold text-gray-700 shadow-sm transition-all hover:bg-gray-50 hover:text-gray-900 focus:ring-4 focus:ring-gray-200/50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto"
                            >
                                Cancelar
                            </button>
                        </div>
